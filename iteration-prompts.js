...
## 驱动代码来源分析
6. 该模块是否有现成的驱动库？（如 OLED 的 SSD1306 驱动、MPU6050 的 DMP 库等）
7. 驱动复杂度如何？（简单寄存器读写 vs 复杂协议栈）
8. 建议：
   - 如果模块有现成驱动库（OLED/TFT/ESP8266/nRF24L01/MPU6050/GPS/SD卡等）→ 建议用户提供驱动代码
   - 如果是简单模块（LED/蜂鸣器/单个传感器）→ 可从模板生成驱动

请给出具体的模块推荐、引脚分配方案，以及驱动代码来源建议。`;
}
